...
- 修复顺序显存模式在解码失败或取消后遗留解码器 GPU 占用的问题；失败清理会恢复可复用的主模型设备状态。
- 性能报告改为显示 INT8 安全回退后的实际加速模式，同时保留原请求模式供排障。
- AUTO 显存提示与模型清单门槛保持一致，已加载模型显示实际选中的显存模式。
- 模型下载器默认支持并固定 `int8-wo-safe-v1` 等四个 Hugging Face 模型 commit，按远端清单预检和校验，不会静默回退 BF16。
- 隔离 Worker 启动日志改用独立锁，提前退出时能返回真实日志；同名模型使用带路径的唯一选项，不再覆盖。
- 新增设备恢复、启动日志、同名模型和下载 revision 回归测试。
