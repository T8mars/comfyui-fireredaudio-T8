...
def build() -> None:
    ...
    review_parser = node(5, "T8_FireRedAudio_ScriptParser", [850, 80], ["1\n00:00:00,000 --> 00:00:02,500\n[旁白] ＡＰＩ 于 2026-08-28 发布。\n\n2\n00:00:02,500 --> 00:00:05,000\n[旁白] 第二句进入逐句审核与返修闭环。", "srt", "旁白", True], [port("voice_bank", "T8_FIREREDAUDIO_VOICE_BANK", 4)], [out("script_plan", "T8_FIREREDAUDIO_SCRIPT_PLAN", [5]), out("normalized_json", "STRING", slot_index=1), out("preflight_report", "STRING", slot_index=2)], [500, 360])
    ...
    review_settings = node(7, "T8_FireRedAudio_GenerationSettings", [430, 80], ["balanced", 42, 750, 6, 512, 10, 2.0], [], [out("settings", "T8_FIREREDAUDIO_SETTINGS", [10, 11])], [330, 280])
    ...
    review_fit = node(9, "T8_FireRedAudio_DurationFit", [2470, 100], ["safe_stretch", 0.1, 1.15, False, 0.9, "v014-review-fit", "fireredaudio/duration-fit"], [port("audio_batch", "T8_FIREREDAUDIO_AUDIO_BATCH", 12)], [out("audio_batch", "T8_FIREREDAUDIO_AUDIO_BATCH", [14, 15]), out("manifest_path", "STRING", slot_index=1), out("retry_line_ids", "STRING", slot_index=2), out("fit_report", "STRING", slot_index=3)], [470, 400])
    ...
    save("api_26_production_review_loop", {
        ...
        "9": {"class_type": "T8_FireRedAudio_DurationFit", "inputs": {"audio_batch": ["8", 0], "strategy": "safe_stretch", "tolerance_seconds": 0.1, "maximum_speed": 1.15, "fit_underrun": False, "minimum_speed": 0.9, "project_name": "v014-review-fit", "subfolder": "fireredaudio/duration-fit"}},
        "10": {"class_type": "T8_FireRedAudio_SpeechQA", "inputs": {"model": ["1", 0], "audio_batch": ["9", 0], "max_text_error_rate": 0.2, "max_clipping_ratio": 0.001, "max_silence_ratio": 0.8, "max_cue_overrun_seconds": 0.5, "max_new_tokens": 512, "use_asr_cache": True, "refresh_asr_cache": False}},
