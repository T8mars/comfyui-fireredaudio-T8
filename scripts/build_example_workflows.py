...
def out(name: str, kind: str, links: list[int] | None = None) -> dict[str, Any]:
    return {"name": name, "type": kind, "links": links or [], "slot_index": 0}
...
def model_node(output_links: list[int]) -> dict[str, Any]:
    return node(
        ...
        [60, 80],
        ["FireRedAudio", "", "cuda:0", "auto", "full", "managed", "", "", "", False, False],
        [],
...
def build() -> None:
    ...
    long_asr = node(3, "T8_FireRedAudio_LongASR", [500, 220], ["Transcribe speech to text.", 30.0, 1.0, 300], [port("model", "T8_FIREREDAUDIO_MODEL", 1), port("audio", "AUDIO", 2)], [out("transcript", "STRING"), out("srt", "STRING"), out("segments_json", "STRING"), out("report", "STRING")], [390, 290])
    save("ui_06_long_audio_asr", workflow([model_node([1]), load_audio, long_asr], [[1, 1, 0, 3, 0, "T8_FIREREDAUDIO_MODEL"], [2, 2, 0, 3, 1, "AUDIO"]]))
    save("api_06_long_audio_asr", {"1": {"class_type": "T8_FireRedAudio_ModelLoader", "inputs": {"model_name": "FireRedAudio", "custom_model_path": "", "device": "cuda:0", "memory_mode": "auto", "profile": "lite", "worker_mode": "managed", "runtime_python": "", "worker_url": "", "worker_token": "", "verify_hashes": False, "release_after_run": False}}, "2": {"class_type": "LoadAudio", "inputs": {"audio": "long_recording.wav"}}, "3": {"class_type": "T8_FireRedAudio_LongASR", "inputs": {"model": ["1", 0], "audio": ["2", 0], "prompt": "Transcribe speech to text.", "chunk_seconds": 30.0, "overlap_seconds": 1.0, "max_new_tokens": 300}}})
    ...
    save("ui_04_voice_design", workflow([model_node([1]), settings, design, save_design], [[1, 1, 0, 3, 0, "T8_FIREREDAUDIO_MODEL"], [2, 3, 0, 3, 1, "T8_FIREREDAUDIO_SETTINGS"], [3, 3, 0, 4, 0, "AUDIO"]]))
