...
NODES = {
    ...
    "T8_FireRedAudio_DurationFit": {
        "zh": ("语音感知字幕时长适配", "让生成语音落入 SRT 时间槽，同时尽量保持自然语速。", ["默认 `speech_aware`：先检测并裁掉首尾多余静音。", "只有剩余超时才用保持音高的 `atempo`；内部停顿不会被裁掉。", "超过最大安全倍率时输出重做 line ID，不截断对白。"], "报告会区分原始所需倍率、首尾裁剪量和残余倍率；源文件始终保留。"),
        "en": ("Speech-aware subtitle duration fit", "Fits generated speech into SRT cue windows while preserving natural delivery where possible.", ["Default `speech_aware` detects and removes excess boundary silence first.", "Pitch-preserving `atempo` is used only for residual overrun; internal pauses are preserved.", "If the residual tempo exceeds the safety limit, the line is sent for regeneration instead of truncation."], "The report separates raw tempo, boundary trims, and residual tempo. Source files are always preserved."),
    },
    ...
    "T8_FireRedAudio_CreativeCandidatePool": {
        "zh": ("单句创意候选池", "为一个现有 line ID 生成不同 Seed 的匿名表演候选，并可把原 Take 一并放入盲听。", ["一次只连接一个 line ID。", "默认保留原 Take，让新候选必须在盲听中真正胜出。", "输出接多 Take 试听评审板，再接采用创意候选。"], "节点不会自动替换原音频；Seed、哈希重复组和文件来源都写入 Manifest。"),
        "en": ("Creative line candidate pool", "Generates anonymous performance variations for one existing line ID and can include the original take in the blind review.", ["Connect exactly one line ID at a time.", "Keep the original take in the pool so a new variation must actually win the listening review.", "Connect to Take Review Board, then Apply Creative Candidate."], "The node never replaces source audio automatically. Seeds, duplicate hash groups, and provenance are recorded in the manifest."),
    },
    "T8_FireRedAudio_TakeReviewBoard": {
        "zh": ("多 Take 试听评审板", "显示候选播放器/下载入口，保存评分、备注和唯一采用项。", ["尽量先盲听再查看 Seed 报告。", "用候选 ID 比序号更稳定。", "评审后的 AudioBatch 和采用 ID 可直接连接候选采用节点。"], "机器推荐只作参考；本节点的明确采用记录才代表人工选择。"),
        "en": ("Take review board", "Shows candidate players/downloads and records ratings, notes, and one adopted take.", ["Listen blind before reading seed evidence when possible.", "Candidate ID is more stable than position.", "Connect the reviewed AudioBatch and selected ID directly to Apply Creative Candidate."], "Automatic recommendations are advisory. This node's explicit adoption record represents the human choice."),
    },
